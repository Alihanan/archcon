...
**Current scope (0.5.16):** a beginner-readable molecular pipeline with a large public **unsupervised dataset**, a smaller kidney-donor **supervised dataset**, reproducible donor-safe outcome-blind pretraining splits, configurable reconstruction-only autoencoders, non-blocking visual exploration, and a molecular/clinical eGFR benchmark with explicit preprocessing provenance.

**0.5.16 donor-safe paper update:** GEO retains its frozen connected-study split of 10,522 train / 585 validation / 584 test arrays. IKEM is gated by donor: 254 measured-eGFR biopsies and four no-eGFR sibling biopsies (`D006_L`, `D037_P`, `D106_P`, `D118_L`) are excluded from pretraining. All 30 donor-clean no-eGFR biopsies are used: 24 train and six validation (`D076_L`, `D076_P`, `D097_L`, `D097_P`, `D184_P`, `D204_L`). Checkpoints and hyperparameters use a predeclared 50% GEO clean-MSE + 50% donor-balanced IKEM clean-MSE score. GEO test is evaluated only for the six frozen preprocessing×architecture winners, and eGFR CV reports those encoders as fixed alternatives without choosing one from outcomes.
...
**0.5.14–0.5.16 downstream-preprocessing update:** the eGFR evaluator no longer sends one common IKEM expression matrix into all six encoders. Global-reference encoders receive IKEM rows mapped with a combined target and probe effects fitted on exactly 10,522 GEO train + 24 IKEM train CELs. Per-dataset-RMA encoders use an IKEM-specific reference fitted on those same 24 IKEM train biopsies; every other IKEM CEL is transform-only. Per-dataset standardization likewise uses only the 24 IKEM train rows. No numeric eGFR value, eGFR-bearing sample, or eGFR CV-fold assignment fits these transformations. Provenance and exact membership hashes are mandatory.

**0.5.13 memory and MetaCentrum update:** `archcon-evaluate-egfr` scans checkpoints on PyTorch's metadata device, retains no model/optimizer tensors in its checkpoint index, memory-maps and evaluates only one checkpoint at a time, and streams R fit results directly to CSV. Newly generated sweep launchers stage the selected matrix, prepared mappings, Python job, results, caches, and checkpoints under node-local `SCRATCHDIR`; after the current 23-hour training budget stops, complete `.pt` files are copied atomically to persistent storage. Submission randomizes unfinished run indices and skips result folders that already contain a checkpoint. The existing persistent virtual environment is used read-only. Standalone reference launchers are flat files under `scripts/`, all named `metacentrum_*`; executable scripts are not placed in the repository root.

**0.5.8 frozen-input update:** the complete molecular training universe is prepared **once when the sweep is generated**. ArchCon aligns the supervised 42,921-probe store to the exact 42,917 canonical GEO probe IDs/order, freezes method-specific GEO row **and probe-column** mappings (so Stadniuk/RMA stores may have different native order), materializes only donor-clean supervised samples with no eGFR, freezes one canonical logical sample order, and saves final `train_rows.npy`, `validation_rows.npy`, and `test_rows.npy`. Generated `run_XXXX.py` jobs only memory-map these prepared arrays; they do **not** reclassify outcomes, compare/reorder probes, remap samples, or recompute a split. Version 0.5.14 retains these frozen inputs while moving active job data and checkpoint writes into node-local scratch.
...
**0.5.9 downstream update:** `archcon-evaluate-egfr` selects encoders by clean validation MSE, freezes them, aligns every supervised expression row to the prepared 42,917-probe order, and exports exact latent arrays plus sample/donor metadata. It then compares a time-only mixed model, the winning z, the best validation-selected alternative architecture, and a fold-fitted PCA baseline under repeated donor-grouped CV. The fitted formulas contain molecular features only; KDRI may stratify folds but never enters the model. Evaluation is exposed through the installed command; the compatibility wrapper lives under `scripts/`, not at repository root.
...
04 Matrix
   samples × 42,917 probes
        ↓
05 Split
   GEO connected-study 10,522/585/584 + donor-clean IKEM 24/6/0
        ↓
06 Molecular AE
...
The MetaCentrum comparison is a **1,440-run CPU grid**. For each of three preprocessing arms — **per-dataset standardization**, **per-study RMA**, and **global RMA** — it evaluates 360 Stadniuk-MLP configurations (180 each at dropout 0.1 and 0) and 120 ResNet-LN configurations. Both MSE and masked-MSE objectives are tested. GEO is split by connected GSE component. IKEM eligibility and its 24/6 split are donor-level, and no IKEM row enters molecular test. The global arm requires the exact CEL-level combined reference; the probe-set Python fallback is not a final-paper substitute.
...
values. Sweep generation first creates a `prepared/` directory containing the 30 aligned donor-clean IKEM rows, method-specific frozen GEO row/probe-column maps, the 24/6 IKEM roles, and final combined train/validation/test integer indices. Each job then only memory-maps those artifacts and the selected GEO matrix. Outcome-bearing and related-donor samples are never placed in the prepared training universe:
...
The final-paper comparison uses the private-CEL `IKEM_CEL_NUMPY_STORE` and the exact combined global reference produced by `scripts/metacentrum_rebuild_geo_rma.pbs.sh`. That reference is fitted on 10,522 GEO + 24 IKEM training CELs and then applied to every held-out array. `archcon-rebuild-global-normalization` remains available for historical probe-set exploration, but its GEO-only fallback is deliberately rejected by 0.5.16 final-paper Global-RMA jobs.
...
`jobs/run_XXXX.py` program. The generated PBS script trains under node-local `SCRATCHDIR`, where
`latest.pt` is atomically replaced after every epoch and `best.pt` on validation improvement.
Its exit handler copies complete checkpoints and the run summary atomically to persistent
`results/run_XXXX/`; resubmission resumes from those persisted files. The CLI imports Gradio only
for browser mode, so PBS batch jobs do not start a web server.
...
├── IKEM_CEL_NUMPY_STORE/       # final-paper 288-row private-CEL IKEM matrices + roles
├── IKEM_NUMPY_STORE/           # optional historical compact IKEM matrix
...
The UI streams running training loss every configurable number of batches, evaluates validation loss/MSE/R² at a configurable epoch cadence, and schedules validation latent-space PCA in a one-worker background executor every configurable number of epochs. Model/training controls are locked while a run is active. Interactive/local jobs update `latest.pt` after each completed epoch and `best.pt` when validation improves. Generated MetaCentrum jobs do the same under node-local scratch and copy complete checkpoint files back atomically when the array element exits. The **Stop** button is cooperative and saves a recovery checkpoint after the current batch.
...
`latest.pt` is replaced after every completed epoch; `best.pt` is replaced only when the declared 50/50 GEO/IKEM validation score improves. Uploading a trusted ArchCon checkpoint restores its architecture/training controls. It can then be used either as weights-only initialization or to resume optimizer/scheduler/history state. Exact resume additionally requires the same preprocessing matrix, train/validation indices, validation domains, and IKEM donor identities; weights-only loading is allowed for a new split or normalization experiment.
...
Run this only after all 1,440 final-paper jobs have completed. The command freezes one
validation winner inside each preprocessing×architecture group before it evaluates the 584
GEO test rows:
...
- every one of the six molecular-validation-frozen preprocessing×architecture embeddings;
- a dimension-matched PCA baseline for each distinct frozen latent dimension, fitted anew inside each training fold;
...
- each fixed-z and matched-PCA model augmented with KDRI or all three clinical predictors.
...
their patient random intercepts are unseen. eGFR outcomes never rank or select the six encoders. Each fixed encoder is a separately reported analysis, with matched-fold uncertainty and baseline comparisons. Use `--no-clinical` for molecular-only models, `--embeddings-only` to stop after exporting z, or `--rebuild-ikem-preprocessing` to invalidate only the cached method-matched IKEM evaluation matrices.

Outputs are written under `sweeps/archcon-pretrain-0516/downstream/molecular_egfr/`:
...
├── embeddings.npz
├── candidate_<preprocessing>_<architecture>_z.npy  # six arrays
└── metadata.json
mixed_models/
...
improve held-out prediction. In `fixed_encoder_comparisons.csv`, `mean_gain` is the matched-fold
`RMSE(baseline) - RMSE(fixed candidate model)`; positive values favor that pre-frozen candidate.
